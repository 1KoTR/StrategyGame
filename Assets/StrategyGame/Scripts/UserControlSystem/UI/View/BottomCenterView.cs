...
public class BottomCenterView : MonoBehaviour
{
    ...
    [SerializeField] private Slider _produceProgressSlider;
    ...
    private Subject<int> _cancelButtonClicks = new();

    private IDisposable _unitProdictionTaskCt;
    ...
    public void Clear()
    {
        ...
        _produceProgressSlider.gameObject.SetActive(false);
        _currentUnitName.text = string.Empty;
        _currentUnitName.enabled = false;
        _unitProdictionTaskCt?.Dispose();
    }
    ...
    public void SetTask(IUnitProductionTask task, int index)
    {
        ...
        else
        {
            ...
            if (index == 0)
            {
                _produceProgressSlider.gameObject.SetActive(true);
                _currentUnitName.text = task.UnitName;
                _currentUnitName.enabled = true;
                _unitProdictionTaskCt = Observable.EveryUpdate().Subscribe(_ => 
                    _produceProgressSlider.value = task.TimeLeft / task.ProductionTime);
            }
        }
    }
}
